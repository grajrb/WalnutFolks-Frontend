...
```bash
git clone https://github.com/yourusername/voice-analytics-dashboard.git
...
src/
├── components/
│   ├── Charts.tsx          # All chart components and dummy data
│   └── EmailModal.tsx      # Email collection and data input form
├── lib/
...
```
User Input → Email Validation → Supabase Query
    ↓
Check Previous Data → Show Previous Values (optional)
    ↓
User Modifies Data → Save to Supabase
    ↓
Update Dashboard Charts → Display Confirmation
```
